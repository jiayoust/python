# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 09:02:41 2019
...
class BoostingTree:
    def __init__(self,epsilon=1e-2):
        ...
        self.cand_splits=[]
        self.split_index = defaultdict(tuple)
        self.split_list=[]
        self.c1_list=[]
        self.c2_list=[]
        self.N=None
    ...
    def init_param(self,X_data):
        ...
        for split in self.cand_splits:
            ...
            right_indx=list(set(range(self.N))-set(left_index))
    ...
    def _cal_err(self,split,y_res):
        ...
        res=n.hstack([y_res_left,y_res_right])
    ...
    def best_split(self,y_res):
        ...
        min_res_square,best_c1,best_c2=self._cal_err(dest_split,y_res)
    ...
    def update_y(self,X_data,y_data):
        ...
        for X, y in zip(X_data,y_data):
            y_res.append(y-self._fx(X[0]))
        y_res = np.apply_along_axis(lambda x: x ** 2, 0, y_res).sum()
...
if __name__=='__main__':
    data = np.array(
        [[1, 5.56], [2, 5.70], [3, 5.91], [4, 6.40], [5, 6.80]
